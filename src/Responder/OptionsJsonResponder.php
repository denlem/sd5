...
use Myitcareer\ServiceBundle\Service\Logger\HealthLogger;
...
class OptionsJsonResponder
{
    ...
    public function __invoke(
        ...
        $statusCode      = $options['statusCode'] ?? self::DEFAULT_STATUS_CODE;
        $errorMessage    = $options['customErrorMessage'] ?? self::DEFAULT_UNKNOWN_ERROR;
        $customErrorCode = $options['customErrorCode'] ?? 0;

        if ($exception instanceof ClientErrorException) {
            $errorMessage = $options['customErrorMessage'] ?? self::DEFAULT_ERROR_MESSAGE_TO_CLIENT_ON_BAD_REQUEST;
        }
